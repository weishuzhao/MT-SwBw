...
```{r}
genome_rltabd %>%
  mutate(Class = taxon.split(get("Taxonomy"), 1, 3)) %>%
  pivot_wider(
    id_cols = "Class", names_from = "Sample",
    values_from = "Relative_abundance", values_fn = sum, values_fill = 0
  ) %>%
  {
    write.table(
      ., "results/tabs/tabS2_class_rltabd.tsv",

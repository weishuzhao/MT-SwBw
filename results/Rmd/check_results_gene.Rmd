...
LastEditTime: 2024-03-04 21:01:32
FilePath: /2021_09-MT10kSW/results/Rmd/check_results_gene.Rmd
Description:
output:
  html_document:
    df_print: paged
...
Wtdb_abd <- stringr::str_glue("Wtdb.relative_abundance.tsv") %>%
...
get_classlocko_ext <- function(
    Pathway = c(), Label = c(), New = c("False"), Class = c()) {
  annotation_genomes_class <-
    merge(Wtdb[c("Genome", "Classification")], annotation_genomes,
      by.x = "Genome", by.y = 0
    ) %>%
    mutate(
      Class = get("Classification") %>% taxon.split(1, 3),
      Classification = get("Classification") %>% taxon.split(1, 7),
    ...
    `colnames<-`(
      colnames(.) %>%
        sapply(
          . %>%
            {
              as.character(key_genes$Label)[which(. == key_genes$KO)]
            }
        )
    ) %>%
...
get_classlocko_ext(Pathway = c("Aromatic"), Class = c(
  "Archaea;Nanoarchaeota;Nanoarchaeia",

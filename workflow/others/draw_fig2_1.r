...
#' @FilePath: /2021_09-MT10kSW/workflow/others/draw_fig2_1.r
#' @Description:
###
...
##### Define function                                                      #####
plot.beta.div <-
  function(type = "metagenome", binary = NA) {
    # >>->> argParse
    cross_rltabd <- cross_rltabd[
      with(sample_meta_cross, get("Sample")[get("Type") == type]),
    ]
    # <<-<<                                                               <<-<<

    dist <- "bray"
    binary <- FALSE
    title_test_method <- "bray distance"
...
set.seed(589)
p1s <-
  sample_meta_cross$Type %>%
  unique() %>%
  `names<-`(., .) %>%
  lapply(function(type) {
    p <-
      plot.beta.div(type = type) +
      theme_bw() +

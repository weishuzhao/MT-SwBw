...
#' @LastEditTime: 2024-02-19 16:10:09
...
genome_jaccard_network <- function(
  ...
  ko_spec <- if (any(is.na(ko_spec))) {
    ...
    ko_spec %>% .[. %in% rownames(genomeko)]
  }
  genome_spec <- if (any(is.na(genome_spec))) {
    colnames(genomeko)
  } else {
    genome_spec %>% .[. %in% colnames(genomeko)]
  }
  ...
  key_genes_spec <- key_genes %>%
    {
      rownames(.) <- .$KO
      .
    } %>%
    .[rownames(genomeko_key), ]

  m_d <- vegen::vegdist(genomeko_key, method = "jaccard", binary = TRUE)
  ...
  plot.igraph(
    ...
    vertex.size = 18,
    vertex.label = key_genes_spec$Label[match(V(g)$name, key_genes_spec$KO)],
    vertex.label.cex = 0.8,
    vertex.label.font = 2,
    vertex.label.family = "Arial",
    vertex.color =
      pathway_col[key_genes_spec$Pathway[match(V(g)$name, key_genes_spec$KO)]],
